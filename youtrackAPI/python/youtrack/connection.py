import calendar
import time
from _elementtree import TreeBuilder, tostring
...
class Connection(object):
    ...
    def m(self, work_item):
        xml = '<workItem>\n'
        xml += '<date>%s</date>\n' % work_item['date']
        xml += '<duration>%s</duration>\n' % work_item['duration']
        if 'description' in work_item:
            xml += '<description>%s</description>\n' % escape(work_item['description'])
        xml += '<worktype>\n'
        if 'worktype' in work_item:
            xml += '<name>%s</name>\n' % work_item['worktype']['name']
            xml += '<url>%s</url>\n' % work_item['worktype']['url']
            xml += '<autoAttached>%s</autoAttached>\n' % work_item['worktype']['autoAttached']
            xml += '<id>%s</id>\n' % work_item['worktype']['id']
        xml += '</worktype>\n'
        xml += '</workItem>\n'
    ...
    def getWorkItems(self, issue_id):
        try:
            response, content = self._req('GET', '/issue/%s/timetracking/workitem' % urlquote(issue_id))
           ...
           # print(xmll)
           # file.close()
            fuckin_json = minidom.parseString(xmll)
